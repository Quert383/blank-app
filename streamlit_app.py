...
def main():
    ...
    for i in range(int(m)):
        ...
        ck = st.number_input(f"Kwota wypłaty nr {i+1}:", key=f"ck_{i}", format="%.2f")
        tk = st.number_input(f"Czas wypłaty nr {i+1} (w latach, np. 0 dla wypłaty dziś):", key=f"tk_{i}", format="%.6f")
    ...
    rata_ostatnia = st.number_input("Wysokość ostatniej raty (wyrównawczej):", min_value=0.0, step=0.01, format="%.2f")

    splaty = []
    # Dodajemy stałe raty co miesiąc
    for i in range(int(liczba_rat_stalych)):
        czas = (i + 1) / 12  # pierwsza rata za 1 miesiąc
